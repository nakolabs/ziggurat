<template>
  <div class="space-y-6">
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
          <div class="flex items-center gap-3">
            ...
            <span class="font-medium text-gray-900 dark:text-white">Grade</span>
          </div>
          <span class="text-xl font-bold text-blue-600 dark:text-blue-400">{{ student?.grade || '-' }}</span>
        </div>
        ...
        <div class="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-xl">
          ...
          <span class="text-xl font-bold text-green-600 dark:text-green-400">{{ subjects?.length || 0 }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 p-6">
      ...
      <div class="space-y-3">
        <div v-if="student?.email" class="flex items-center gap-3 p-2 hover:bg-gray-50 dark:hover:bg-neutral-800 rounded-lg transition-colors">
          <Mail class="w-4 h-4 text-gray-400" />
          <span class="text-sm text-gray-600 dark:text-gray-300">{{ student.email }}</span>
        </div>
        <div v-if="student?.phone" class="flex items-center gap-3 p-2 hover:bg-gray-50 dark:hover:bg-neutral-800 rounded-lg transition-colors">
          <Phone class="w-4 h-4 text-gray-400" />
          <span class="text-sm text-gray-600 dark:text-gray-300">{{ student.phone }}</span>
        </div>
        <div v-if="!student?.email && !student?.phone" class="text-center py-4">
